  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center" style={{ height: height || 400 }}>
        ...
        <Text className="text-medium-emphasis mt-2">Loading...</Text>
      </View>
    );
  }
  ...
  if (error) {
    return (
      <View className="flex-1 items-center justify-center p-4" style={{ height: height || 400 }}>
        <Text className="text-high-emphasis text-lg mb-2">Error loading carousel</Text>
        <Text className="text-medium-emphasis text-center mb-4">{error.message}</Text>
        <Pressable onPress={refetch} className="bg-brand-mid px-4 py-2 rounded">
